...
async function getStravaAccessToken() {
  ...
  if (missing.length) {
    throw new Error(
      `Missing ${missing.join(', ')} in Vercel Environment Variables`
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      `Strava token failed: ${JSON.stringify(tokenData)}`
    );
  }
...
async function fetchActivities(accessToken, limit, page = 1) {
  ...
  if (!response.ok) {
    throw new Error(
      `Strava activities failed: ${JSON.stringify(data)}`
    );
  }
...
function normalizeActivity(a) {
  ...
  return {
    ...
    durationMin: Math.round(
      (a.moving_time || a.elapsed_time || 0) / 60
    ),
...
module.exports = async (req, res) => {
  try {
    ...
    current.workouts = Array.isArray(current.workouts)
      ? current.workouts
      : [];
    ...
    await writeGithubData(
      current,
      `Import Strava activities (${added} added, ${updated} updated)`
    );
